...
class Sfm:
    def __init__(self, img_dir: str, downscale_factor: float = 1.0) -> None:
        ...
        parser = argparse.ArgumentParser(
            description="Parameters for the 3D reconstruction"
        )
        parser.add_argument(
            "-d", type=int, default="1", metavar="number", help="Number of drones"
        )
        parser.add_argument(
            "-r", type=float, default="0.5", metavar="ratio", help="Ratio of the circle"
        )
        parser.add_argument(
            "-t",
            type=int,
            default="1",
            metavar="trajectory",
            help="Predefined or dynamic trajectory",
        ...
        parser.add_argument(
            ...
            default="1",
            metavar="position",
            help="Get or estimate camera position",
        )  # 1 = Get for the drons, 2 = Estimate position
        args = parser.parse_args()
        self.img_final = os.getcwd()

        self.n_drones = args.d
        self.trajectory = args.t
        self.positioning = args.p
        self.ratio = args.r
        self.z_distance = args.z

        print(self.n_drones, self.trajectory, self.positioning)

        #self.dron = Dron()
        #self.cam = AIDECK("Test_1", 5000, "172.20.10.4")
...
"""
    ...
    def reconstruction(self, dir, id):
        if id == 1:
            ...
            ini = 0
        # only to get the tranform matrix from the file
        dir_2 = "images/Test_1"
        n_points = 45
        max_points_quality = 30000
        ...
        if False == False:
            ...
            while True:
                ...
                thread = threading.Thread(
                    target=self.dron.do_next_point, args=(seq, 0, stop_event, get_pos)
                )
                thread.start()
                ...
                imgs_2 = sorted(
                [file for file in os.listdir(dir_2) if file.lower().endswith(".png")]
                )'''
                print(len(imgs))
                ...
                imgs = sorted(
                    [file for file in os.listdir(dir) if file.lower().endswith((".jpg",".png"))]
                )
                '''stop_event.set()
                self.dron.do_stop_event(stop_event)
                thread.join()'''
                ...
                if (actual>max_points_quality or finish == 0):
                    ...
                    print("Acaba", (time.time() - start - (ac_time - start_time)))
                    break
