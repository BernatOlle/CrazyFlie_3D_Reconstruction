...
# from rep_drone_3d import Drone
...
class Sfm:
    def __init__(
        self, img_dir: str, downscale_factor: float = 1.0, n_drones: int = 1
    ) -> None:
        """
        Initialise and Sfm object.
        """

        with open("pose_array.pickle", "rb") as file:
            self.pose_array_f = pickle.load(file)

        with open("transform_matrix_1_array.pickle", "rb") as file:
            self.transform_array_f = pickle.load(file)

        with open("tran_matrix_final.pickle", "rb") as file:
            self.tran_matrix_final = pickle.load(file)

        self.img_obj, self.img_dir = [], []
        self.img_final = os.getcwd()

        for i in range(n_drones):
            self.img_obj.append(Image_loader(img_dir + str(i + 1), downscale_factor))
            self.img_dir.append(img_dir + str(i + 1))

        print("DIR" + str(self.img_dir))

    def downscale_image(self, image, id):
        """
        ...
        """
        for _ in range(1, int(self.img_obj[id].factor / 2) + 1):
    ...
    def divide_point_cloud(self, points, center_x, center_y, center_z):
    ...
    def calculate_point_cloud_center(self, point_cloud):
    ...
    def to_ply(self, path, point_cloud, colors, name) -> None:
        """
    ...
    def to_show(self, point_cloud):
        x, y, z = self.calculate_point_cloud_center(point_cloud)
    ...
    def recal(self, imgs, image_2, file_1, vect, id):
        t_points_3d = []

        max = 0
        pose_array = self.img_obj[id].K.ravel()
        print("ID", id, vect.keys())
        down = 0

        for k, file in enumerate(imgs[2:-2], start=2):
            print("ID", id, "Recal", file)
            dir = "images/" + self.img_dir[id]
            file_path = os.path.join(dir, file)
            print("ID", id, file_path)

            img1 = cv2.imread(file_path)
            image_1 = self.downscale_image(img1, id)
            image_1 = self.remove_bg(image_1)

            print("ID", id, "Insert new points recal", k)
            pose_0, pose_1, feature_0, feature_1 = vect[k]
            ...
            try:
                ...
                pose_2 = np.matmul(self.img_obj[id].K, transform_matrix_1)
                ...
                print("ID", id, "Reprojection Error: ", error)
                print("ID", id, "New points: ", len(points_3d))
                # print("Dimensions recal final:", points_3d.ndim)

            except Exception as e:
                print("ERROR recal fin", e)
                continue
            print("ID", id, "Length t_points", len(t_points_3d))

            if len(t_points_3d) < len(points_3d):
                print(
                    "ID",
                    id,
                    "total " + str(len(t_points_3d)) + " curr " + str(len(points_3d)),
                )
            ...
            if down == 1:
                ...
                break
            print("ID", id, "Down " + str(down))
            print()
        return ret_cm_mask_1, t_points_3d

    def reconstruction(self, dir, id):
        if id == 1:
            ini = 30
        else:
            ini = 0
        ...
        # self.drone.conn_drone()
        print("start", id)
        rotation = 0
        pose_0 = np.matmul(self.img_obj[id].K, transform_matrix_0)
        ...
        print(imgs)
        print(self.tran_matrix_final.keys())
        i = 0
        # pose_0 = self.pose_array_f[i + ini]
        vect = {}
        start_time = time.time()
        # self.drone.initial_run()
        while True:
            # go to the point i of the sequence, so until there is not a new photo the drone
            # will not go to the next position
            # if i < 54:
            # print("Photo")
            # self.cam.take_photo()
            # more_sequence = self.drone.next_point(self.drone.cf,self.drone.sequence,i)
            ...
            if len(imgs) > i:
                ...
                if file_0.lower().endswith(".jpg"):
                    ...
                    if i < 2:
                        ...
                        if i > 0:
                            print("ID", id, "nomes al primer cop")
                            ...
                            tran_matrix = self.tran_matrix_final[i + ini + id][0]
                            rot_matrix = self.tran_matrix_final[i + ini + id][1]
                            ...
                            # transform_matrix_1 = self.transform_array_f[i + ini]

                            pose_1 = np.matmul(self.img_obj[id].K, transform_matrix_1)
                            # pose_1 = self.pose_array_f[i + ini]
                            ...
                            # p = mp.Process(target=self.to_show, args=(index, copia_points_3d, nrows, ncols, rotation, final, fig,))
                            # p.start()
                    ...
                    img1 = cv2.imread(file_path)
                    image_2 = self.downscale_image(img1, id)
                    ...
                    try:
                        ...
                        # pose_2 = self.pose_array_f[i + ini]
                        ...
                        print("ID", id, "Reprojection Error: ", error)
                        print("ID", id, "New points: ", len(points_3d))
                        pose_array_cal.append(pose_2)
                        pose_array = np.hstack((pose_array, pose_2.ravel()))
                        if len(points_3d) < 200:
                            cm_mask_1, points_3d = self.recal(
                                imgs, image_2, file_0, vect, id
                            )
                    ...
                    file_1 = file_0
                    vect[i] = [pose_0, pose_1, feature_0, feature_1]
            ...
            if (ac_time - start_time) > 10:
                ...
                """with open("tran_matrix_final.pickle"+str(id), "wb") as file:
        ...
        # while
        # self.drone.go_land(self.drone.cf, self.drone.initial_values)
        # p.join()
    ...
    def __call__(self):
        ...
        for i, cur_dir in enumerate(dir):
            threads.append(
                threading.Thread(target=self.reconstruction, args=(cur_dir, i))
            )
            threads[i].start()
        ...
        center_x, center_y, center_z = self.calculate_point_cloud_center(
            self.final_point
        )

        quadrants = self.divide_point_cloud(
            self.final_point, center_x, center_y, center_z
        )
...
if __name__ == "__main__":
    sfm = Sfm("GustavIIAdolf", n_drones=2)
    sfm()
    print("Final")
