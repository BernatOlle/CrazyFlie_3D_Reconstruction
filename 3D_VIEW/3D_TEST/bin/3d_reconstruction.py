...
class Sfm:
    ...
    def __call__(self, enable_bundle_adjustment: boolean = False):
        ...
        while True:
            ...
            # break
            point = seq[1]
            thread = threading.Thread(target=self.dron.do_next_point, args=(seq,0,stop_event))
            thread.start()
            if len(imgs) > i:
                print("Inici",i)
                file_0 = imgs[i]
                if file_0.lower().endswith(".jpg"):
                    if i < 2:
                        ...
                        self.dron.do_stop_event(stop_event.set())
                        thread.join()
